...
- `execute <plan>` → dispatch a cheaper executor subagent on one plan in an ignored workspace-local disposable worktree, defaulting to `plans/.worktrees/<plan-id>-<slug>/` with `plans/.gitignore` preserving the ignore entry, then review its diff like a tech lead — audit the done-criteria report, check scope, read the code — and render a verdict that reports the worktree path and branch. **Never copy files or re-run commands in the main workspace; the worktree diff is the proof.** Treat the executor's diff as untrusted until reviewed: verify every hunk traces to a plan step and reject any out-of-scope change, however plausible it looks. Requires a host agent that can spawn subagents in an isolated worktree or run them from the prepared worktree; if yours can't, say so and hand the plan over for manual execution instead. Detailed mechanics live in [references/closing-the-loop.md](references/closing-the-loop.md); read it before the first dispatch.
